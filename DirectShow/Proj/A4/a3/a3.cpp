...
//Front mic input
	...
	bstrDeviceName = SysAllocString(L"ÏßÂ· (USB Multimedia Audio Device");// device name as seen in Graphedit.exe
...
//Default output device
	DEVICE_CLSID = CLSID_WMAsfWriter;// the audio renderer device category
	bstrDeviceName = SysAllocString(L"ASF Writter");// device name as seen in Graphedit.exe
	cout<<pDeviceMonik<<endl;
	pDeviceMonik = Device_Read(pDeviceEnum,pDeviceMonik,DEVICE_CLSID,bstrDeviceName);//read the required device
