...
//Default Output device
	...
	bstrDeviceName = SysAllocString(L"ÑïÉùÆ÷ (USB Audio Device)");// device name as seen in Graphedit.exe
	pDeviceMonik = Device_Read(pDeviceEnum,pDeviceMonik,DEVICE_CLSID,bstrDeviceName);//read the required device
...
//Get the information about the Capture device and media
	...
	pInputDevice->FindPin(L"1",&pOutPin);


	pOutputDevice->FindPin(L"Audio Input pin (rendered)",&pInPin);
